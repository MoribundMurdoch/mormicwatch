...
pub fn classify(
    ...
    if last_signal_at.elapsed()
        > Duration::from_secs(NO_SIGNAL_TIMEOUT_SECS)
    {
        return MicStatus::NoSignal;
    }
...
pub fn calculate_health(
    ...
    if !device_connected {
        score = 0;
        reasons.push(
            "Recording device disconnected".into(),
        );
    }
    ...
    if clipping {
        ...
        reasons.push("Clipping detected".into());
    }

    if rms_db < -60.0 {
        score = score.saturating_sub(10);
        reasons.push(
            "Input level extremely low".into(),
        );
    }
